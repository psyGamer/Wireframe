...
public class InternalWorld extends InternalBlockReader implements World {
	...
	public boolean setBlockState(final BlockState blockState, final BlockPosition position, final UpdateFlag updateFlag) {
		return this.internalWorld.setBlock(McNativeKt.getMcNative(position), blockState.getMcNative(),
										   updateFlag.getInternalFlag()
		);
	}
	
	@Override
	public boolean isReplaceable(final BlockPosition position) {
		final net.minecraft.block.BlockState blockState = this.internalWorld.getBlockState(McNativeKt.getMcNative(position));
	...
	public void notifyNeighbours(final BlockPosition position) {
		this.internalWorld.updateNeighborsAt(
				McNativeKt.getMcNative(position),
				
				this.internalWorld.getBlockState(McNativeKt.getMcNative(position))
								  .getBlock()
		);
	}
	...
	public void notifyNeighbours(final BlockPosition position, final dev.psygamer.wireframe.block.Block block) {
		this.internalWorld.updateNeighborsAt(McNativeKt.getMcNative(position), block.getMcNative$Wireframe_main());
	}
	...
	public boolean breakBlock(final BlockPosition position, final boolean dropItems) {
		return this.internalWorld.destroyBlock(McNativeKt.getMcNative(position), dropItems);
	}
	
	@Override
	public boolean isAir(final BlockPosition position) {
		return this.internalWorld.isEmptyBlock(McNativeKt.getMcNative(position));
	}
	
	@Override
	public boolean isBlock(final dev.psygamer.wireframe.block.Block block, final BlockPosition position) {
		return this.internalWorld.getBlockState(McNativeKt.getMcNative(position))
								 .getBlock() == block.getMcNative$Wireframe_main();
	}
	
	@Override
	public boolean isLoaded(final BlockPosition position) {
		return this.internalWorld.isLoaded(McNativeKt.getMcNative(position));
	}
	
	@Override
	public float getBlockLightLevel(final BlockPosition position) {
		return this.internalWorld.getBrightness(LightType.BLOCK, McNativeKt.getMcNative(position));
	}
	
	@Override
	public float getSkyLightLevel(final BlockPosition position) {
		return this.internalWorld.getBrightness(LightType.SKY, McNativeKt.getMcNative(position));
	}
