...
public class FerrusUtil {
	public static ModDefinition<?> getDependant(final String classPath) {
		for (final ModDefinition<?> dependant : FerrusCore.getDependants()) {
	...
	public static String getFirstExternalClass() {
		return Arrays.stream(Thread.currentThread().getStackTrace())
				.filter(FerrusUtil::isPartOfLibrary)
				.findFirst()
	...
	public static boolean isPartOfLibrary(final String className) {
		return !className.startsWith(FerrusCore.Constants.FERRUS_PACKAGE) &&
